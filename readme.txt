...
- There are 3 Python2 files in the directory "python", "training.py" and "generation.py" can be run from the console, "prepMidiData.py" will only 
  be used as a helper file for the other 2

- To run, the python files will require you have modules "keras", "scandir", "numpy" and "music21" and maybe some others installed, 
...
- Running the python file "training.py" will train the neural network on a given data set (specified at the top of "prepMidiData.py") and write 
  ...
  "beethoven-S100-B128-L0.4146.hdf5" in the "weights" directory
...
- Running the python file "generation.py" will use this checkpoint file (specified at the top of the file) and the data set to create a new 
  midi file named "new_sound.mid" (also specified at the top of the file), this step can be repeated with the same checkpoint file for a new 
  different midi file, although be careful of overwritting previously generated music with more files named the same

- The folder "my-generated" contains some MIDI files I have generated earlier (also like blue peter) using the "beethoven" data set that I liked, 
  using some of the training data provided
